...
class _ShowCartState extends State<ShowCart> {
  ...
  Future<Null> orderThead() async {
    ...
    String nameFood = nameFoods.toString().trim();
  ...
  Future<Null> notificationToShop(String idShop) async {
    ...
    await Dio().get(urlFindToken).then((value) {
      ...
      for (var json in result) {
        ...
        String title = 'Have Order';
        String body = 'Have order from user.';
        String urlSendToken =
            '${MyConstant().domain}/aishoppingmall/apiNotification.php?isAdd=true&token=$shopToken&title=$title&body=$body';
  ...
  Future<Null> sendNotificationToShop(String urlSendToken) async {
    await Dio().get(urlSendToken).then(
          (value) => normalDialog(context, 'Send Order to shop is done.'),
        );
  }
}
